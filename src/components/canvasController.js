...
export class CanvasController {
  constructor(canvas, isMain = false) {
    ...
    this.tex_main = this.igloo
      .texture(null, gl.RGBA, gl.REPEAT, gl.LINEAR, gl.FLOAT, gl.RGBA32F)
      .blank(this.viewsize[0], this.viewsize[1]);

    this.tex_temp1 = this.igloo
      .texture(null, gl.RGBA, gl.REPEAT, gl.LINEAR, gl.FLOAT, gl.RGBA32F)
      .blank(this.viewsize[0], this.viewsize[1]);

    if (isMain) {
      this.tex_temp2 = this.igloo
        .texture(null, gl.RGBA, gl.REPEAT, gl.LINEAR, gl.FLOAT, gl.RGBA32F)
        .blank(this.viewsize[0], this.viewsize[1]);
      this.tex_temp3 = this.igloo
        .texture(null, gl.RGBA, gl.REPEAT, gl.LINEAR, gl.FLOAT, gl.RGBA32F)
        .blank(this.viewsize[0], this.viewsize[1]);
      this.tex_temp4 = this.igloo
        .texture(null, gl.RGBA, gl.REPEAT, gl.LINEAR, gl.FLOAT, gl.RGBA32F)
        .blank(this.viewsize[0], this.viewsize[1]);
    }
  ...
  _getMousePos(e) {
    ...
    return [(currentX - rect.left) * resolutionCoeff, (height - (currentY - rect.top)) * resolutionCoeff];
  }
  ...
  zoom(scale, inPlace = true) {
    const u_matrix = [scale, 0, 0, scale];
    return this._operation(this.program_matrix, {}, {}, { u_matrix }, inPlace);
  }
  ...
  dft() {
    ...
    //    ↓
    // [DFT X pass]
    //    → temp1 (real), temp2 (imag)
    //    ↓
    // [DFT Y pass]
    //    → temp3 (real), temp4 (imag)
    //    ↓
    // [Polar conversion]
    //    → magnitude, phase
    //    ↓
    // [Log scaling]
    //    → displayable magnitude
